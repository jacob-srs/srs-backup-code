...
import getopt
# import svgfig
# from r".vscode\20191118_dualCam\PatternGenerator\svgfig" import *
from svgfig import *
...
class PatternMaker:
    def __init__(
        self, cols, rows,
        output, units, square_size,
        radius_rate, page_width, page_height
    ):
        self.cols = cols
        self.rows = rows
        self.output = output
    ...
    def makeACirclesPattern(self):
        ...
        for i in range(0, self.rows):
            for j in range(0, self.cols):
                dot = SVG(
                    "circle",
                    cx=((j * 2 + i % 2)*spacing) + spacing,
                    cy=self.height - (i * spacing + spacing),
                    r=r,
                    fill="black",
                    stroke="none"
                )
